...
def check_if_twamp_session_active(config_db, name):
    """ Check if TWAMP-Light session status in the state db """
    entry = config_db.get_entry(CFG_TWAMP_SESSION_TABLE_NAME, name, 'role')
    if entry is None:
        return False
    if entry['role'] != TWAMP_ROLE_SENDER:
        return False

    state_db = SonicV2Connector(host='127.0.0.1')
    state_db.connect(state_db.STATE_DB, False)
    entry = state_db.get_entry(STATE_TWAMP_SESSION_TABLE_NAME, name)
    if entry is None:
        return False
...
def validate_twamp_session_exist_cb(ctx, param, name):
    ...
    if check_if_twamp_session_exist(config_db, name) is True:
        raise click.UsageError('Invalid value for "<{}>": {}. TWAMP-Light session already exist'.format(param.name, name))
    return name
...
def validate_twamp_session_cb(ctx, param, name):
    ...
    if name == 'all':
        ...
        if len(session_keys) == 0:
            raise click.UsageError('Invalid value for "<{}>": {}. TWAMP-Light session not exist'.format(param.name, name))
        return session_keys
    else:
        session_keys = name.split(",")
        for key in session_keys:
            if check_if_twamp_session_exist(config_db, key) is False:
                raise click.UsageError('Invalid value for "<{}>": {}. TWAMP-Light session not exist'.format(param.name, key))
        return session_keys
...
def validate_twamp_ip_port_cb(ctx, param, ip_port):
    ...
    if ip_port.count(':') == 1:
        ...
        if check_twamp_udp_port(udp_port) is False:
            raise click.UsageError('Invalid value for "<{}>": {}. Valid udp port range in 862|863|1025-65535'.format(param.name, udp_port))
    else:
        ip_addr = ip_port
    ...
    try:
        ip = ipaddress.ip_interface(ip_addr)
    except ValueError as e:
        raise click.UsageError('Invalid value for "<{}>": {}. Valid format in IPv4_address or IPv4_address:udp_port'.format(param.name, ip_addr))
...
@click.argument('packet_count', metavar='<packet_count>', required=True, type=click.IntRange(min=100, max=30000), default=100)
@click.argument('tx_interval', metavar='<tx_interval>', required=True, type=click.Choice(['10', '100', '1000']), default='100')
@click.argument('timeout', metavar='<timeout>', required=True, type=click.IntRange(min=1, max=10), default=5)
@click.argument('statistics_interval', metavar='<statistics_interval>', required=False, type=click.IntRange(min=2000, max=3600000))
